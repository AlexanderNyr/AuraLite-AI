...
- **⚡ INT8 Quantization (NEW)**: One-click weight-only INT8 quantization of native AuraLite models (per-output-channel symmetric scales, llama.cpp Q8_0-style). ~4× smaller `.pt` checkpoints and memory footprint with minimal quality loss; works on CPU and CUDA with zero extra dependencies. Quantized checkpoints save/load transparently. Quantized models are inference-only — keep the FP32 checkpoint if you plan to fine-tune later. Available via the **⚡ Quantize INT8** button on the Model tab or `engine.quantize_model("int8")` in code.
